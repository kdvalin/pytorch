...
static void testCudaTestVectorAdd01_impl() {
  ...
  ctype* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, N * sizeof(ctype)));
  ctype* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, N * sizeof(ctype)));
  ctype* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, N * sizeof(ctype)));
  C10_CUDA_CHECK(
...
TEST(Cuda, Sigmoid_CUDA) {
  ...
  // TODO: move gpu support into PaddedBuffer
  float* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, N * sizeof(float)));
  float* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, N * sizeof(float)));
  C10_CUDA_CHECK(
...
static void testCudaTestVectorAdd02_impl(int64_t N, int64_t block_size) {
  ...
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, N * sizeof(float)));
  float* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, N * sizeof(float)));
  float* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, N * sizeof(float)));
  C10_CUDA_CHECK(
      cudaMemcpy(a_dev, a_v.data(), N * sizeof(float), cudaMemcpyHostToDevice));
...
TEST(Cuda, HalfCast_CUDA) {
  ...
  C10_CUDA_CHECK(cudaMallocManaged(&aDev, aSize));
  C10_CUDA_CHECK(cudaMallocManaged(&bDev, bSize));
...
TEST(Cuda, DynamicShape2D_CUDA) {
  auto testWithSize = [](int32_t M, int32_t N) {
    ...
    float* bDev = nullptr;
    float* cDev = nullptr;
    C10_CUDA_CHECK(cudaMallocManaged(&aDev, aData.size() * sizeof(aData[0])));
    C10_CUDA_CHECK(cudaMallocManaged(&bDev, bData.size() * sizeof(bData[0])));
    C10_CUDA_CHECK(cudaMallocManaged(&cDev, cData.size() * sizeof(cData[0])));
...
TEST(Cuda, OneBlockOneThreadGlobalReduce1_CUDA) {
  ...
  C10_CUDA_CHECK(cudaMallocManaged(&data_dev, N * sizeof(float)));
  C10_CUDA_CHECK(cudaMemcpy(
      data_dev, data_v.data(), N * sizeof(float), cudaMemcpyHostToDevice));
  float* output_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&output_dev, 1 * sizeof(float)));
  C10_CUDA_CHECK(cudaDeviceSynchronize());
...
TEST(Cuda, OneBlockMultiThreadGlobalReduce1_CUDA) {
  ...
  C10_CUDA_CHECK(
      cudaMemcpy(a_dev, a_v.data(), N * sizeof(float), cudaMemcpyHostToDevice));
  float* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, 1 * sizeof(float)));
  C10_CUDA_CHECK(cudaDeviceSynchronize());
...
TEST(Cuda, NoThreadIdxWrite_1_CUDA) {
  ...
  float* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, 2 * sizeof(float)));
  float* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, N * sizeof(float)));
  C10_CUDA_CHECK(cudaDeviceSynchronize());
...
# CHECK: atomicAdd(&b[blockIdx.x], c_1)
  ...
  float* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, kTotalSize * sizeof(float)));
  C10_CUDA_CHECK(cudaMemcpy(
...
TEST(Cuda, HalfSupport_CUDA) {
  ...
  C10_CUDA_CHECK(cudaMallocManaged(&aDev, aSize));
  C10_CUDA_CHECK(cudaMallocManaged(&bDev, bSize));
  C10_CUDA_CHECK(cudaMallocManaged(&cDev, cSize));
  C10_CUDA_CHECK(cudaMallocManaged(&dDev, dSize));
  C10_CUDA_CHECK(cudaMemcpy(aDev, aData.data(), aSize, cudaMemcpyHostToDevice));
...
# CHECK: })IR";
  ...
  C10_CUDA_CHECK(cudaMallocManaged(&aDev, aSize));
  C10_CUDA_CHECK(cudaMallocManaged(&reluDev, reluSize));
  C10_CUDA_CHECK(cudaMemcpy(aDev, aData.data(), aSize, cudaMemcpyHostToDevice));
  C10_CUDA_CHECK(
...
TEST(Cuda, PrioritizeDependents_CUDA) {
  ...
  float* b_dev = nullptr;
  float* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, 10 * sizeof(float)));
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, 12 * sizeof(float)));
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, 12 * sizeof(float)));
...
# CHECK:   d[blockIdx.x] =)IR";
  ...
  float* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, A_SIZE * sizeof(float)));
  float* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, B_SIZE * sizeof(float)));
  float* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, A_SIZE * sizeof(float)));
  float* d_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&d_dev, B_SIZE * sizeof(float)));
  C10_CUDA_CHECK(cudaMemcpy(
...
# CHECK:   D[threadIdx.x + 50 * blockIdx.x] =)IR";
  ...
  float* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, OUTER_SIZE * A_SIZE * sizeof(float)));
  float* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, OUTER_SIZE * B_SIZE * sizeof(float)));
  float* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, OUTER_SIZE * A_SIZE * sizeof(float)));
  float* d_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&d_dev, OUTER_SIZE * B_SIZE * sizeof(float)));
  C10_CUDA_CHECK(cudaMemcpy(
      a_dev,
      a_v.data(),
...
# CHECK:   D[B_SIZE * int64_t(blockIdx.x) + int64_t(threadIdx.x)] =)IR";
  ...
  float* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, OUTER_EXTENT * A_EXTENT * sizeof(float)));
  float* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, OUTER_EXTENT * B_EXTENT * sizeof(float)));
  float* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, OUTER_EXTENT * A_EXTENT * sizeof(float)));
  float* d_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&d_dev, OUTER_EXTENT * B_EXTENT * sizeof(float)));
  C10_CUDA_CHECK(cudaMemcpy(
      a_dev,
      a_v.data(),
...
# CHECK:     D[threadIdx.x + 15 * blockIdx.x] =)IR";
  ...
  float* a_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&a_dev, OUTER_A_SIZE * A_SIZE * sizeof(float)));
  float* b_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&b_dev, OUTER_B_SIZE * B_SIZE * sizeof(float)));
  float* c_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&c_dev, OUTER_A_SIZE * A_SIZE * sizeof(float)));
  float* d_dev = nullptr;
  C10_CUDA_CHECK(cudaMallocManaged(&d_dev, OUTER_B_SIZE * B_SIZE * sizeof(float)));
  C10_CUDA_CHECK(cudaMemcpy(
      a_dev,
      a_v.data(),
